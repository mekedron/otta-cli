...
func newAbsenceBrowseCommand() *cobra.Command {
	var (
		dateFrom     string
		dateTo       string
		user         string
	...
	cmd := &cobra.Command{
		Use:   "browse",
		Short: "Browse absences across a date range.",
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			report, err := collectAbsenceRange(cmd.Context(), client, fromDate, toDate, user, order, sideload)
			if err != nil {
				return err
			}
