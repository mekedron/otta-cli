...
- Keep `origin` on SSH only: `git@github.com:<github-owner>/otta-cli.git`.
- If `origin` is changed, immediately restore it with:
  - `git remote set-url origin git@github.com:<github-owner>/otta-cli.git`
...
- Minimum required commands:
  - `go mod download`
  - `go build ./...`
  - `BUILD_VERSION="$(git describe --tags --always --dirty)" && go build -trimpath -ldflags "-s -w -X main.version=${BUILD_VERSION}" -o bin/otta ./cmd/otta`
